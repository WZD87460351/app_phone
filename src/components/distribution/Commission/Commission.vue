    ...
    <div class="main">
        ...
        <div class="container">
            <div class="top-bar">
                ...
                <router-link tag="span" :to="{name:'comwithdrawals'}">{{set.commission_detail}}</router-link>
            </div>

            <ul>
                <li>
                    <span>{{set.commission_ok}}</span>
                    <span class='text-danger'>{{member.commission_ok}}{{set.yuan}}</span>
                </li>
            </ul>
            ...
            <ul>
                ...
                <li v-if="member.commission_charge > 0"><span>{{set.commission_charge}}</span><span>{{member.commission_charge}}{{set.yuan}}</span>
                </li>
            </ul>
            ...
            <ul>
                ...
                <li class="txt" v-if="isShow">
                    <div v-if="set.settledays > 0">
                        <p>买家确认收货（<span class='text-danger'>set.settledays天</span>)后，{{set.commission}}可{{set.withdraw}}
                            。结算期内，买家退货，{{set.commission}}将自动扣除。</p>
                        <p v-if="set.withdraw > 0">注意：可用{{set.commission}}满<span class='text-danger'>{{set.withdrawset.yuan}}</span>后才能申请{{set.withdraw}}
                        </p>
                    </div>
