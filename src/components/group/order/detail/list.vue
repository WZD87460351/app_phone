<template>
    <div class="list-box">
        <div class="list">
            <div>商品小计</div>
            <div>￥{{order.is_team=='1'?goods.groupsprice:goods.singleprice}}</div>
        </div>
        <div class="list">
            <div>运费</div>
            <div>￥{{goods.freight}}</div>
        </div>
        <!--<div class="list" v-if="goods.deductenough>0">-->
            <!--<div>满额立减</div>-->
            <!--<div>￥-{{goods.deductenough}}</div>-->
        <!--</div>-->
        <div class="list" v-if="order.heads==1&&order.discount>0&&order.ishead==1">
            <div>团长优惠</div>
            <div class="act">-￥{{order.discount}}</div>
        </div>
        <div class="list">
            <div>实付费</div>
            <div class="act">￥{{order.amount}}</div>
        </div>
    </div>
</template>
