    ...
    <transition name="slide" v-if="silde">
        <div class="main">
            ...
            <div class="container">
                ...
                <div class="base-info">
                    ...
                    <div class="price" v-if="!goodsInfo.seckillinfo">
                        ...
                        <div v-else>
                         ...
                         <!--分享-->
                         <div class="iconfont icon-fenxiang" @click="goshare"></div>
                    </div>
                    <!-- 限时购 -->
                    ...
                    <div class="btm">
                        <span v-if="dispatchtype == 'object'">快递：{{goodsInfo.dispatchprice.min+'~'+goodsInfo.dispatchprice.max}}</span>
                        <span v-else>快递：{{goodsInfo.dispatchprice == 0 ? '包邮' : goodsInfo.dispatchprice}}</span>   
                ...
                <div v-if="goodsInfo.seckillinfo ==''" style="padding-bottom:.1rem;">
                    ...
                    <div class="vip-discount" v-if="goodsInfo.isdiscount == 0 || goodsInfo.isdiscount != 0 && goodsInfo.isdiscount_time < now">
                        <p v-if='!goodsInfo.memberprice ==  ""&& goodsInfo.memberprice.price != goodsInfo.minprice && !goodsInfo.levelbuy == 0'>
                ...
                <div class="activity-ct" v-if="goodsInfo.ispresell == 1 && (goodsInfo.preselltimestart < now && goodsInfo.preselltimeend > now || goodsInfo.preselltimeend == 0)">
                    ...
                    <div>
                        <p v-if="goodsInfo.preselltimeend > 0">结束时间：{{goodsInfo.isdiscount_time2}}</p>
                ...
                <!-- 商品详情 -->
                <div class="title-tab" v-if="goodsInfo.content || (params && params.title)">
                    <div :class=" tabIndex == 0? 'cur' :''" @click="tab(0)">商品详情</div>
                    <div :class=" tabIndex == 1? 'cur' :''" @click="tab(1)">参数</div>
                </div>
                <div class="detail-box">
                	<div v-if="tabIndex==0&&thumb_first==1">
            ...
            <div class="fixed-btm" v-if="goodsInfo.canbuy">
                <div class="left">
                    <router-link tag="div" class="list" :to="{name:'index'}">
                        <!--<div class="iconfont">&#xe67c;</div>-->
                        <div class="iconfont">&#xe89f;</div>
                        <p>首页</p>
                    </router-link>
...
<script>
    ...
    export default {
        data() {
            return {
                popupVisible:false,//本页是否弹出 购买窗口
                silde:true,
                cartcount:'',//购物车红色角标数量
                tabIndex:0,//tab切换
                num: 1, //选择购买的数量
                ...
                if (new_arr.length == _this.spec.length) {
                    ...
                    for (let o in options) {
                        if (options[o].specs === optionAll) {
                            ...
                            _this.zero =  _this.stock <= 0 ? 0 : 1;
                            _this.marketprice = changeOptions.marketprice
                            _this.opitiontitle= (new_arr).join("+");;
                            _this.selectoption='已选：';
                ...
                util.request({
                    ...
                    success(res) {
                        ...
                        _this.thumb = result.thumb;
                         _this.thumb_first = result.thumb_first;
                        _this.isfavorite = result.isfavorite;
                        //默认库存：总库存
                        _this.zero = result.total <= 0 ? 0 : 1;
                        _this.stock = result.total;
                        //快递类型
                        _this.dispatchtype = typeof(result.dispatchprice);
//                      console.log('result.dispatchprice:', typeof(result.dispatchprice))
            ...
            getSpec(){
                ...
                util.request({
                    ...
                    success(res){
                        ...
                        if(res.data.options !== undefined && res.data.specs !== undefined){

                            _this.opitions = res.data.options;
                            _this.spec = res.data.specs;

                            if( res.data.specs.length == 0 && res.data.options.length == 0){
                                //不需要选择规格,直接购买
                                _this.buyRight = true;
                            }
                        }else{ //商品没有规格
            ...
            favorite(){
                let _this = this;
                let id = _this.$route.query.id;
                let isfavorite = _this.isfavorite == true ? 0 : 1
                util.request({
                    ...
                    method: 'get',
                    success(res){
                        _this.isfavorite =  res.data.isfavorite ? true : false
                        if(_this.isfavorite){
                            Toast({
                                message: '收藏成功',
                                position: 'middle',
            ...
            init(){
                ...
                promise.then(this.getSpec).then(this.getCart).catch(()=>{
                    _this.buyRight = true;
                });
            },
            goshare(){
