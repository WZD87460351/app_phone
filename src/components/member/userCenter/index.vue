            ...
            init() {
                ...
                util.request({
                    ...
                    success: function (res) {
                        ...
                        if(res.data.commission ===true && res.data.commission_text === '分销中心' && that.commission ===false){
//                            that.$set(that.item,
                            let o={
                                icon: '&#xe681;',
                                text: '分销中心',
            ...
            checkShop(){
                ...
                util.request({
                    ...
                    success(res) {
                        ...
                        if (!res.data.reg) {
                            // 申请
                            params.router = 'settled'
                        } else if (res.data.reg.status === 0) {
                            //审核中
                            params.router = 'settled'
                        } else if (res.data.reg.status === -1) {
                            //被驳回 重新提交
                            params.router = 'settled'
                        } else if (res.data.reg.status === 1) {
                            //店铺页
                            params.router = 'multipleindex'
                        }
                        _this.item.unshift(params)
